...
K8S 기본 정책상 마스터 노드는 Taint 설정이 되어있어, 파드가 스케줄링되지 않도록 되어있다.

```bash
kubectl describe node <마스터노드이름> | grep -i taint # 마스터 노드 Taint 확인
```
...
```yaml
kubectl taint nodes ip-172-31-45-126.ap-northeast-2.compute.internal node-role.kubernetes.io/control-plane:NoSchedule-
```
...
![K8S Dashboard Login Page]({{site.url}}/assets/img/High-Volume-Traffic-15/15-K8S_Dashboard_Login.png)
...
![K8S Dashboard Page]({{site.url}}/assets/img/High-Volume-Traffic-15/16-K8S_Dashboard_UI.png)
...
![K8S Master Node Join]({{site.url}}/assets/img/High-Volume-Traffic-15/12-Master_Node_Join.png)
...
![K8S Master Node]({{site.url}}/assets/img/High-Volume-Traffic-15/13-Node_List_1.png)
...
kubeadm-config 설정에서 NodeCluster구성에서 Node IP를 Elastic IP의 Public IP를 이용으로 인해 발생하였다. 노드 간 통신은 인스턴스 내부 네트워크를 통해 이루어지기에 `Elastic IP`를 `Private IP`로 설정해야한다.

![K8S Master Node 2]({{site.url}}/assets/img/High-Volume-Traffic-15/14-Node_List_2.png)
