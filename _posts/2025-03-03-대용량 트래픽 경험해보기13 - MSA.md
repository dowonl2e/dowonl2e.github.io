...
기존 여행 플래너 서비스에서 서비스 확장을 생각하면서 **`숙소 조회 및 예약`**에 대해 전략적 설계를 **`유비쿼터스 언어 정의 → 이벤트 스토밍`** 순 진행했다.
...
![이벤트 스토밍 - 커멘드,액터,외부 시스템 도출]({{site.url}}/assets/img/High-Volume-Traffic-14/4-Event_Storming_Command_Actor_System.png)

<!-- ### **문장으로 액터, 커멘드, 이벤트 검토**

- `예약자(액터)`{: .text-yellow}는 `숙소검색(커맨드)`{: .text-blue}하면 `숙소조회됨(이벤트)`가 발생하고 이어서 외부 시스템 `숙소(시스템)`이 실행된다.
- `예약자(액터)`{: .text-yellow}는 `예약(커맨드)`{: .text-blue}하면 `예약요청됨(이벤트)` `예약생성됨(이벤트)` `결제됨(이벤트)` `결제상태변경됨(이벤트)` 가 발생한다.
  - `시스템(액터)`{: .text-yellow}는 `가용성확인(정책)`으로 `가용성확인(커맨드)`{: .text-blue}하면 `가용성확인됨(이벤트)`가 발생하고, `예약(정책)`으로 `예약(커맨드)`{: .text-blue}하면  `예약됨(이벤트)`가 발생하며, 이어서 외부 시스템 `숙소(시스템)`이 실행된다.
- `예약자(액터)`{: .text-yellow}는 `예약취소(커맨드)`{: .text-blue}하면 `예약취소요청됨(이벤트)` `예약상태변경(이벤트)` `환불됨(이벤트)` `결제상태변경됨(이벤트)`가 발생한다.
  - `시스템(액터)`{: .text-yellow}는 `예약취소(정책)`으로 `예약취소(커맨드)`하면 `예약취소됨(이벤트)`가 발생하며, 이어서 외부 시스템 `숙소(시스템)`이 실행된다.
- `시스템(액터)`{: .text-yellow}는 `알림발송(정책)`으로 `알림발송(커맨드)`{: .text-blue}하면 `알림발송됨(이벤트)`가 발생하고 이어서 외부 시스템 `알림(시스템)`이 실행된다. -->
...
- 숙소 시스템은 숙박시설별 PMS(Property Management System)을 사용하여 CMS(Channel Manage System)을 이용하게된다. CSM 시스템 사용에 대한 협업을 할 수 없기에 유사한 CSM 시스템을 별도로 개발
- 결제 시스템(PG)의 경우 모든 PG 시스템을 연동하여 구현하기에 제한적인 부분이 있다. Toss API를 이용하여 간단한 결제 과정을 이용을 목적으로 진행하지만, Toss Core API 사용에 상점관리자 설정이 불가능하다는 점에서 결제 처리 API 응답 객체를 기반으로 도메인 설계
- 기존의 여행 플래너 서비스에서 회원과 여행 플래너 각각의 도메인으로 분리
- MSA 과정에서 기존 Nginx가 아닌 Spring Cloud Gateway를 이용해 클라이언트와 서비스간에 Gateway를 구현
...
API Gateway는 기존 Nginx과 비교했을 때 **`Gateway 기능에 최적`**이며 **`필터링을 통한 인증처리`** 를 위해  **`Spring Cloud Gateway`**를 이용하려 한다.
