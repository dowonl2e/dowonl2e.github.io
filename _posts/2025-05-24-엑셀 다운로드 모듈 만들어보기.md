---
title: 엑셀 다운로드 모듈 만들어보기 (Apache POI)
# author: dowonl2e
date: 2025-05-24 04:00:00 +0800
...
> 클래스 구성을 기반으로 **`생성 패턴으로는 빌더 패턴`{: .text-blue}** **`행동 패턴으로는 전략 패턴`{: .text-blue}** **`구조 패턴은 퍼사드 패턴`{: .text-blue}**을 이용하기로 했다.
...
CellStyleApplier customCellStyleApplier = CellStyleApplier.builder()
    ...
    .background(c -> c.color(IndexedColorValues.PALE_BLUE))
    .fontStyleApplier(fontStyleApplier)
    .build();
...
Workbook에서는 스타일 및 폰트 생성, 시트 조회, 스타일 수 조회 등의 다양한 옵션이 있고, Sheet의 경우 40개 이상의 설정이 가능하다. 해당 옵션을 **ExcelExporter**에 모두 담기에는 해당 클래스의 복잡성이 증가하다는 점에서 각 설정 및 기능들을 개별 클래스에 구현했으며, **ExcelExporter**에서는 필요 메소드를 제공하는 방식으로 **`퍼사드 패턴`{: .text-blue}**을 이용하였다.

```java
public abstract class ExcelExporter {

  private final WorkbookBuilder workbookBuilder; // Workbook의 세부 기능을 담당하는 클래스
...
// 3. 커스텀 객체 데이터 구성
for (MemberResponseDto member : members) {
    exporter.row().writeTargetCells(member);
}
```
...
1. 엑셀 다운로드에 구현에 필요한 코드를 시트, 스타일, 헤더, 셀 랜더링 코드를 심플하게 구현할 수 있어 편의성과 가독성이 좋아진 것 같다.
    ...
    → **`함수형 인터페이스를 이용한 전략 패턴`{: .text-blue}** **`빌더 패턴`{: .text-blue}** **`템플릿 메소드 패턴`{: .text-blue}** **`퍼사드 패턴`{: .text-blue}**을 이용
